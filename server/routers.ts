  ...
  poi: router({
    ...
    checkIn: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Verify proximity (within 200m using haversine formula)
        const poiLat = typeof poi.latitude === 'string' ? parseFloat(poi.latitude) : poi.latitude;
        const poiLon = typeof poi.longitude === 'string' ? parseFloat(poi.longitude) : poi.longitude;
        
        const R = 6371; // Earth radius in km
        const dLat = ((input.latitude - poiLat) * Math.PI) / 180;
        const dLon = ((input.longitude - poiLon) * Math.PI) / 180;
        const a =
        ...
        if (distance > 200) {
          throw new TRPCError({ code: "BAD_REQUEST", message: `Too far from POI (${(distance).toFixed(0)}m away, max 200m)` });
        }
